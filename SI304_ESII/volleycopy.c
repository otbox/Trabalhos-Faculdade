...
void teams(int numerosTimes)
{
    ...
    {
        printf("Time %d) colocacao: %d jogador %s \n", (numerosTimes-1)-(i%numerosTimes), jogadores[i].nivel, jogadores[i].nome);
        char temp[300]; 
        sprintf(temp, "%d %s", jogadores[i].nivel, jogadores[i].nome);
        strcat(TimesValidos[(numerosTimes-1)-(i%numerosTimes)], temp);
        // strcat(TimesValidos[(numerosTimes-1)-(i%numerosTimes)], jogadores[i].nivel);
    }
